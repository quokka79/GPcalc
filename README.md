...
The GP folder will also contain a tab-delimited text file containing histogram values for each image. There are three set of x and y data here.

The three x-axes are:
- 'Intensity', the range of possible pixel intensity values (in an 8 bit image this is 256 values ranging 0-255).
- 'GP', the pixel intensity values converted to GP values (this will also be 256 values but now the range is -1 to +1).
- 'GP corrected', the GP values corrected by the G-Factor. The range will be the same as for the GP values if your G-Factor is 1.0 but otherwise the axis values will adjust accordingly and the distribution of the data should stay the same.

The three y-axes are:
- 'Counts', the number of pixels with values matching each histogram bin.
- 'Smooth', a kernel-smoothed histogram. This also sets the extreme values (GP = -1 and GP = +1) to zero as these bins often dominate, especially in masked images. The remaining values are an average of each bin and it's adjacent neighbours.
- 'Norm Av Dist', a variation on the smoothed histogram where the values are the normalized to the mean GP value. The sum of these values should be 1.

When plotting histograms to see the distribution of GP values, pick a pair of x and y values from the .tsv file. In general you will look at plotting 'GP' vs 'Counts' for a standard frequency histogram. You may need to manually limit the y-axis in cases where the majority of your pixels have one value, e.g. in masked images where everything 'outside' the mask is -1.
